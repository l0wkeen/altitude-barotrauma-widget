...
- 고도 변화 단계에 따라 이관가압 완화 행동(물 마시기, 하품, 발살바법)을 알림으로 안내
  (3단계 알림은 펼치면 발살바 방법이 표시되고, 앱 안에서도 "예방 행동 방법 보기"로 확인 가능)
...
- **고도 보정**: GPS 좌표로 지형 고도 API를 조회하는 자동 보정, 또는 실제 고도 직접 입력(수동 보정)
- **테스트 알림** 버튼(1·2·3단계): 각 단계의 실제 경고와 동일한 알림(제목·문구·증상 기록 버튼 포함)을 즉시 띄워 동작 확인·스크린샷용으로 사용
...
| `ElevationApi` | 위도·경도로 지형 고도를 조회하는 헬퍼(Open-Meteo Elevation API) — 자동 보정용 |
...
- **같은 위치에서도 고도 표시가 흔들림**: 기압계 기반 고도는 실제 이동이 없어도 주변 기압이
  변하면 값이 함께 변합니다. 예를 들어 10m 변화는 약 1~1.2hPa의 기압 변화에 해당하는데,
  날씨 변화·실내 냉난방(HVAC)·문 여닫힘·바람 등으로 짧은 시간 안에도 흔히 발생하는 크기입니다.
  등산용 고도계가 주기적으로 "기준점 재설정"을 요구하는 것과 같은 이유입니다. 이 앱은 GPS 등
  외부 기준으로 재보정하는 기능이 없어 이런 흔들림을 그대로 보여줍니다. 다만 알림 로직은
  절대 고도가 아니라 1분 누적/3초 즉각 **변화량**만 사용하므로, 이 정도의 자연스러운 흔들림은
  현재 임계값(90m 이상)에는 거의 영향을 주지 않습니다.
- **절대 고도 표시 오차**: 고도 계산은 실제 해수면 기압이 아니라 표준대기압(1013.25hPa)을
  기준으로 하므로, 화면의 "현재 고도" 절대값 자체도 날씨에 따라 실제 고도와 차이가 날 수 있습니다.
  이 오차는 **고도 보정 기능**으로 줄일 수 있습니다: MainActivity에서 "자동 보정"(GPS 좌표 →
  Open-Meteo 지형 고도 API로 실제 고도 조회 → 오프셋 저장) 또는 "수동 보정"(실제 고도 직접 입력)을
  적용하면 이후 표시 고도에서 오프셋이 반영됩니다. 보정은 절대 고도 표시에만 영향을 주며,
  변화량 기반 알림 로직은 오프셋이 상쇄되어 영향을 받지 않습니다.
